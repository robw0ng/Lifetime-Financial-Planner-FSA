...
async function simulateScenario(scenarioId) {
	...
	const life_expectancy =
		scenario.life_expectancy_type === "fixed"
			? scenario.life_expectancy_value
			: sampleNormal(scenario.life_expectancy_mean, scenario.life_expectancy_std_dev);
	const spouse_life_expectancy = scenario.is_married
		? scenario.spouse_life_expectancy_type === "fixed"
			? scenario.spouse_life_expectancy_value
			: sampleNormal(scenario.spouse_life_expectancy_mean, scenario.spouse_life_expectancy_std_dev)
		: 0;
	...
	for (let year = currentYear; year < endYear; year++) {
		...
		if (state.is_married && year >= spouseEndYear) {
			...
			const incomeOrExpenseEvents = state.events.filter(
...
async function processRMD(state, scenario, year) {
	...
	const preTaxInvestments = state.investments.filter((inv) => inv.tax_status === "pre-tax" && inv.value > 0);
	...
	for (const investmentId of rmdStrategy) {
		...
		let targetInv = state.investments.find(
			(inv) => inv.investment_type_id === sourceInv.investment_type_id && inv.tax_status === "non-retirement"
		);
...
async function processRothConversion(state, scenario, year) {
	...
	for (const investmentId of rothStrategy) {
		...
		let targetInv = state.investments.find(
			(inv) => inv.investment_type_id === sourceInv.investment_type_id && inv.tax_status === "after-tax"
		);
...
async function processNonDiscretionaryExpensesAndTax(state, scenario, year, startYear, yearData, returnData) {
	...
	if (year > startYear) {
		...
		if (state.currentTaxBrackets.state) {
			...
			if (applicableBracket) {
				...
				stateTax =
					applicableBracket.baseTax + (remainingIncome - applicableBracket.from) * applicableBracket.rate;
			}
		}
		...
		const capitalGainsTax = calculateCapitalGainsTax(
			state.prevYearGains,
			state.currentTaxBrackets.capitalGains,
			state.is_married
		);
	...
	if (totalAssets < totalPayment) {
		console.log(
			`Simulation stopped in year ${year}: Not enough assets to pay non-discretionary expenses and taxes`
		);
		return returnData;
	}
...
function calculateCapitalGainsTax(gains, brackets, isMarried) {
	...
	const applicableBrackets = brackets
		.filter((bracket) => {
			if (isMarried) {
				...
				return (
					bracket.filingStatus === "single;" || bracket.filingStatus === "single married filing separately;"
				);
			}
		})
	...
	for (const bracket of applicableBrackets) {
		const gainsInBracket = Math.min(Math.max(0, remainingGains - bracket.from), bracket.to - bracket.from);
...
async function processInvestEvents(state, year) {
	...
	const investEvents = state.events.filter(
		(event) =>
			event.type === "invest" &&
			year >= event.start_year_value &&
...
function sampleInflationRate(scenario) {
	switch (scenario.inflation_assumption_type) {
		...
		case "normal":
			return sampleNormal(scenario.inflation_assumption_mean, scenario.inflation_assumption_std_dev);
		case "uniform":
			return sampleUniform(scenario.inflation_assumption_lower, scenario.inflation_assumption_upper);
	}
}

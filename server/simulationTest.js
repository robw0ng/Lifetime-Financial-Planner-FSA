...
async function createTestScenario() {
	...
	const cashType = await InvestmentType.create({
		...
		is_locked: false,
		expected_change_numtype: "amount",
		expected_income_numtype: "amount",
	});
	...
	const spType = await InvestmentType.create({
		...
		expected_change_type: "fixed",
		expected_change_value: 1,
		expense_ratio: 0.001,
		expected_income_type: "normal",
		expected_income_mean: 1,
		expected_income_std_dev: 0.5,
		taxability: true,
		scenario_id: scenario.id,
		is_locked: false,
	...
	const bondType = await InvestmentType.create({
		...
		expense_ratio: 0.004,
		expected_income_type: "normal",
		expected_income_mean: 3,
		expected_income_std_dev: 1,
		taxability: false,
		scenario_id: scenario.id,
		is_locked: false,
		expected_change_numtype: "amount",
	...
	await ExpenseEventSeries.create({
		...
		expected_change_mean: 2,
		expected_change_std_dev: 1,
	...
	const vacationEvent = await EventSeries.create({
		name: "vacation",
		type: "expense",
		start_year_type: "yearAfterEvent",
		start_year_other_event: "salary",
		duration_type: "fixed",
		duration_value: 40,
		scenario_id: scenario.id,
...
async function runTest() {
	try {
		...
		const dbScenario = await Scenario.findOne({
		...
		if (!dbScenario) throw new Error("Scenario not found");

		const newscenario = dbScenario;
		console.log("Running simulation...");
		await simulateScenario(newscenario.id);
